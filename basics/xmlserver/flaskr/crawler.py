...
def getStartSeite():
    clearCookies()
    return execCurl("flaskr/startseite.curl")
...
def performLogin():
    ...
    return execCurl("flaskr/login.curl", r)
...
def get_xml(roomname):
    ...
    if not os.path.exists(tempfile):
        ...
        agenda = execCurl("flaskr/generic.curl", [["MYURL",menu['Agenda']]])
        menu = getNavMenu(agenda)                  
        raumplan = execCurl("flaskr/generic.curl", [["MYURL",menu['Raumpläne']]])
        ...
        roomplan = execCurl("flaskr/generic.curl", [["MYURL",menu['Raumpläne']+f"&listindex_s={room}&subFilter="]])


    source = html.tostring(roomplan).decode("utf-8")
    ...
    return execCurl("flaskr/generic.curl", [["MYURL",paramurl]], False)
