...
def draw_data(current_weekday, current_date, event_date, starttime, endtime, subject, Class, teacher, aditional_info, time, subject_short, teacher_short, weekday, reservator, drawbw, font, bw, text, drawrw, rw):
    ...
    print(text)
    slot = (weekday-current_weekday+7)%7
    ...
    if text != "Reserviert":
        if (current_weekday==weekday):
            ...
            draw_lesson(drawbw, subject_short, Class, teacher_short, aditional_info, time, slot, anfangszeiten, font)
    else:
        if starttime in anfangszeiten:
            draw_reservation_at_lessontime(drawbw, starttime, teacher, time, slot, anfangszeiten, font, drawrw)
        else:
            draw_reservation(drawbw, starttime, endtime, teacher, time, slot, anfangszeiten, current_weekday, font, drawrw)
